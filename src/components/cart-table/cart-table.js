...
const CartTableContainer = ({ carts, onDecrement, onIncrement, onDelete }) => {

    const renderRaw = (item) => {
        const { id, brand, model, year, price, count, total } = item
        return (
            <tr key={id}>
                <td>{brand}</td>
                <td>{model}</td>
    ...
    return (
        <div className="shopping-cart-table">
            ...
            <div className="total-count">
                <span className="total-item">Total</span><span className="total-value">2000</span>
            </div>
        </div>
    )
}
